...
| 2026-07-26 | 4 | complete | `034_mmo_road_base_encounters.sql` + `evaluateRoadBaseEncounters`/`expireRoadBaseEncounters` close the milestone 2 gap noted above: a moving expedition within `roadcombat.EncounterRadius` of a real player's (non-AI) base now has the same `EncounterRollChance` to open a forced Attack/Continue window, not just a passive discovery. Only the expedition's commander gets a choice (the base didn't choose to be in the road's path). Attack resolves immediately against the base's Manual Defense Garrison reserve (`workshop_inventory.garrisoned_soldiers`/`garrisoned_mechs`) as a `roadcombat.FieldForce` - deliberately no resource looting, so a full economic raid still requires the normal `/raid` flow with its existing protections (shields, pacts). Kept as a separate `road_base_encounters` table/`active_base_encounter_id` column rather than widening `road_encounters`, so the expedition-vs-expedition path is untouched. AI/seeded settlements are excluded (their own recon/skirmish flow; this predates the Phase 6 AI civilizations work merged afterward). |
| 2026-07-26 | 6 | partial (foundational tier) | Corrects the stale "6-7 pending" note this row replaces, written before this session's Phase 6 work merged: 8 persistent AI factions (2 per continent) exist as real `encampments` rows (synthetic negative-telegram_id `users` row satisfying the existing FK), fully discoverable/raidable/lootable through the existing pipeline with zero special-casing, and grow passively (`growAICivilizations`). AI factions do not yet make decisions, scout, or launch their own raids - see the Phase 6 completed-implementation-detail section below for the exact boundary. |
| 2026-07-26 | 7 | pending | Observability/balance tooling remains to be implemented. |
| 2026-07-26 | - | bugfix + exploration rework | Asiwaju reported exploration effectively never surfacing a discovery ("takes a decade... just resources... crystal or ether"). Root cause found: exploration was a single shared, contested site per continent (`exploration_sites`) - one player claims it, every other concurrent player in that continent got nothing until a new one randomly spawned at only 15%/tick. With many concurrent players, most never even reached the 35%+ discovery roll, no matter how many times they tried. Fixed: exploration is now personal and always available per outpost - every dispatch generates its own site on the spot, nothing shared, nothing to race. Reward variety expanded from 4 currencies (metal/crystal/ether/dollars) to all 9 real resources, weighted by rarity (common materiel frequent, Crystal/Ether rare finds). Discovery-chance base raised 0.35->0.45 (cap 0.75->0.80) for extra headroom now that the real bottleneck is fixed - a fresh outpost should now see roughly a 92% chance of at least one first-contact within 5 expeditions. Separately, a requested combat bug sweep of `resolveRaidCombats` found two real, previously-undiscovered bugs: (1) an active `acid_rain` weather event mutated the shared `totMechs` variable in place, which silently corrupted every downstream use of it for the rest of that raid's resolution - casualty-pool distribution, the reported attacker casualty count, the Mechs count shown in the battle report, and co-op split ratios; fixed by scoping the penalty to a dedicated `mechOffenseCount` used only in the attack-rating formula. (2) In this session's own Phase 4 road-battle code, the winner-determination logic compared `result.AWon` (which already meant "the attacker's side won," since `ResolveBattle` was called with that side as its "A" argument) against the unrelated `raid_a`/`raid_b` canonical ordering used only for the encounter table's unique index - silently flipping the winner, and therefore the cargo-capture direction and the "WON"/"LOST" battle-report headline for both sides, whenever the attacker happened to be `raid_b` in that ordering. Fixed by using `result.AWon` directly. Note: this fix was first written against a slightly stale local branch state and had to be re-applied fresh once `git push` revealed the branch had diverged (another session had merged Phase 4 milestone 2 and three waves of UI polish in the meantime) - re-verified against the actual current `main` before this commit, per the standing "verify real git state, don't trust a stale local view" lesson logged earlier in this ledger. |
